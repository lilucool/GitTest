  ...
  WXFSInit: function (ttl, capacity) {
          ...
          if (temp !== undefined) {
            if(temp.cleanable && unityNamespace.isAndroid && temp.time + this.ttl * 1000 < Date.now()){
              try {
                ...
                GameGlobal.manager.reporter.wxAssetBundle.reportEmptyContent({stage: WXABErrorSteps['kCacheGet'], path: check_path, error: !!e ? e.toString() : 'unknown'});
    ...
    WXFS.LoadBundleFromFile = function(path){
      ...
      if(!res || res.byteLength != expected_size){
        var wxab_error = {
          stage: WXABErrorSteps['kLoadBundleFromFile'],
          path: path,
